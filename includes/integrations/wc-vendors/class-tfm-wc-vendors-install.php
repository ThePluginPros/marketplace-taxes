...
class TFM_WC_Vendors_Install {
    ...
    /**
     * @var TFM_WC_Vendors_Order_Updater Updater instance.
     */
    protected static $order_updater = null;

    public function __construct() {
        add_action( 'tfm_load_integration', array( $this, 'init_order_updater' ), 5 );
    ...
    public function init_order_updater() {
        require_once __DIR__ . '/class-tfm-wc-vendors-order-updater.php';

        self::$order_updater = new TFM_WC_Vendors_Order_Updater();
    }
    ...
    public function check_sub_orders() {
        if ( get_option( 'tfm_sub_orders_checked' ) ) {
            return;
        }

        if ( 0 < sizeof( $this->get_orders_needing_update() ) ) {
            TFM_WC_Vendors_Admin_Notices::add_notice( 'sub_order_update' );
        }

        update_option( 'tfm_sub_orders_checked', true );
    }
    ...
    public function maybe_update_sub_orders() {
        if ( isset( $_GET['tfm_update_sub_orders'] ) && ! self::$order_updater->is_updating() ) {
            $this->update_sub_orders();
        }

        if ( isset( $_GET['tfm_force_update_orders'] ) ) {
            $this->update_sub_orders();
    ...
    protected function update_sub_orders() {
        ...
        foreach ( $this->get_orders_needing_update() as $order_id ) {
            $logger->info( sprintf( 'Queueing %d', $order_id ), [ 'source' => 'tfm_wcv_order_updates' ] );
