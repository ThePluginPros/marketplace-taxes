...
class MT_Install {
    ...
    public function __construct() {
        add_action( 'marketplace_taxes_activated', array( $this, 'maybe_install' ) );
    ...
    public function maybe_install() {
        if ( false === get_option( 'mt_version' ) ) {
            $this->install();
        }
    }
    ...
    public function install() {
        ...
        update_option( 'mt_version', MT()->version );
    }
    ...
    public function check_tax_rates() {
        ...
        if ( ! get_option( 'mt_rate_warning_dismissed' ) ) {
            $dismissed = false;
            ...
            if ( isset( $_GET['keep_rates'] ) ) {
                ...
                if ( 'no' === $_GET['keep_rates'] ) {
                    ...
                    $message     = sprintf(
                        __( '%1$s tax rate(s) deleted successfully.', 'marketplace-taxes' ),
                        $num_deleted
                    );
                } else {
                    $message = __( 'Your existing tax rates will be kept.', 'marketplace-taxes' );
                }

                MT()->admin->add_notice( 'warning_dismissed', 'success', $message );

                update_option( 'mt_rate_warning_dismissed', true );
            }
    ...
    public function tax_rates_warning() {
        ...
        $message = sprintf(
            __(
                '<strong>Warning!</strong> There are tax rates in your WooCommerce tax tables. This may cause your customers to be overtaxed. Please choose to <a href="%1$s">keep</a> or <a href="%2$s">delete</a> the existing tax rates.',
                'marketplace-taxes'
            ),
