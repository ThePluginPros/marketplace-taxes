 ...
 * WooCommerce :(
 */
class TFM_Refund_Uploader extends WP_Background_Process {
    ...
    public function __construct() {
        ...
        $this->action = 'tfm_refund_uploader';

        add_action( 'taxjar_for_marketplaces_activated', array( $this, 'enable_or_disable' ) );
        add_action( 'taxjar_for_marketplaces_deactivated', array( $this, 'disable' ) );
        add_action( 'taxjar_for_marketplaces_options_saved', array( $this, 'handle_settings_changes' ) );
        add_action( 'tfm_update_refund_queue', array( $this, 'update_queue' ) );
    ...
    public function set_merchant_of_record() {
        $this->merchant_of_record = TFM()->settings->get( 'merchant_of_record' );
    }
    ...
    public function enable() {
        ...
        if ( ! $this->is_enabled() ) {
            wp_schedule_event( time(), 'daily', 'tfm_update_refund_queue' );
        }
    }
    ...
    public function disable() {
        $timestamp = wp_next_scheduled( 'tfm_update_refund_queue' );

        if ( $timestamp ) {
            wp_unschedule_event( $timestamp, 'tfm_update_refund_queue' );
        }
    ...
    public function update_queue() {
        ...
        if ( ! $this->needs_update() ) {
            $logger->info( 'Skipping update.', [ 'source' => 'tfm_refund_uploader' ] );
            return;
        }
        ...
        // the loop as needed
        $api_token = TFM()->settings->get( 'api_token' );
        ...
        foreach ( $refunds as $refund ) {
            if ( $this->should_upload( $refund ) ) {
                ...
                $refund->update_meta_data( '_tfm_upload_status', 'skipped' );
            }
    ...
    protected function needs_update() {
        ...
        if ( 'marketplace' === $this->merchant_of_record ) {
            return ! empty( TFM()->settings->get( 'api_token' ) );
        }
    ...
    protected function get_from_address( $refund ) {
        ...
        if ( $vendor_id ) {
            $address = TFM()->integration->get_vendor_from_address( $vendor_id );
        } else {
            $address = [
    ...
    protected function get_to_address( $refund ) {
        ...
        return apply_filters( 'tfm_refund_to_address', $address, $refund );
    }
    ...
    protected function increment_upload_attempts( &$refund ) {
        $current_attempts = $refund->get_meta( '_tfm_upload_attempts', true );

        if ( ! is_numeric( $current_attempts ) ) {
            $current_attempts = 0;
        }

        $refund->update_meta_data( '_tfm_upload_attempts', $current_attempts + 1 );
        $refund->update_meta_data( '_tfm_last_attempt', time() );
    }
    ...
    protected function should_upload( $refund ) {
        if ( 'vendor' === $this->merchant_of_record ) {
            $min_date = get_user_meta( $refund->get_meta( '_vendor_id', true ), 'tfm_reports_start_date', true );
        } else {
            $min_date = get_option( 'tfm_reports_start_date' );
        }
        ...
        return apply_filters( 'tfm_upload_refund', $refund->get_date_created() >= $min_date, $refund );
    }
...
WHERE 
  ...
  AND m1.meta_key = 'tfm_upload_transactions' 
  AND m1.meta_value = 'yes' 
  AND m2.meta_key = 'tfm_taxjar_api_token' 
...
SQL;
    ...
    protected function task( $item ) {
        ...
        try {
            ...
            $attempts = $refund->get_meta( '_tfm_upload_attempts' );
        ...
        $refund->update_meta_data( '_tfm_upload_status', $success ? 'succeeded' : 'failed' );
        $refund->save();
    ...
    public function filter_orders_query( $query, $query_vars ) {
        ...
        $meta_query = [
            ...
            [
                ...
                [
                    'key'     => '_tfm_upload_attempts',
                    'value'   => $this->get_max_attempts(),
                    'compare' => '<',
                    'type'    => 'NUMERIC',
                ],
                [
                    'key'     => '_tfm_last_attempt',
        ...
        return apply_filters( 'tfm_refund_uploader_orders_query', $query, $query_vars );
    }
    ...
    public function is_enabled() {
        return false !== wp_next_scheduled( 'tfm_update_refund_queue' );
    }
    ...
    protected function get_max_attempts() {
        return apply_filters( 'tfm_max_upload_attempts', 3 );
    }
